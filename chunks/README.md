...
```bash
# Pretty print JSON
cat chunks/document_chunks.json | python -m json.tool

# Count chunks
cat chunks/document_chunks.json | grep -o '"text"' | wc -l
```
...
rm chunks/specific_document_chunks.json
```
...
```bash
# Backup all chunks
tar -czf chunks_backup_$(date +%Y%m%d).tar.gz chunks/
...
1. Delete the file: `rm chunks/problematic_document_chunks.json`
...
- `backend/rag/document_manager.py` - Manages chunk saving/loading
- `backend/rag/loader.py` - Extracts and chunks PDFs
- `backend/MetaData.json` - Tracks document metadata
- `backend/uploads/` - Source PDF files
